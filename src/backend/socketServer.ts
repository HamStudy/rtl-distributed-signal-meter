...
async function saveData(
  ...
  const { TestRunData, TestRun } = await getCollections(
    "testRunData",
    "testRun"
  );
...
router.ws("/connectNode/:nodeId", async (ws, req) => {
  const { NodeStatus } = await getCollections(
    "nodeStatus",
    "testRun",
    "testRunData"
  );
    ...
    switch (msg.type) {
      ...
      case "data":
        ...
        if (
          ...
          dataDoc.timestamp > nodeStatusDoc.rfStatus.updatedAt
        ) {
          nodeStatusDoc.rfStatus = {
          ...
          NodeStatus.updateOne(
            { _id: nodeStatusDoc._id },
            {
